...
class ConvEncoder(nn.Module):
    def __init__(self, in_channels, resolution):
        ...
        self.encoderConv3 = nn.Sequential(
            BasicConv2d(in_channels=64, out_channels=128,
                        kernel_size=3, padding=1, stride=1),
            nn.MaxPool2d(2)
        )
        ...
        self.fc = nn.Sequential(
            nn.Linear(self.width*self.width*128, 500),
            nn.ReLU(),
            nn.Linear(500, 10)
        )
...
class ConvDecoder(nn.Module):
    def __init__(self, out_channels, resolution):
        ...
        self.fc = nn.Sequential(
            ...
            nn.Linear(2000, 500),
            nn.ReLU(),
            nn.Linear(500, self.width*self.width*128),
            nn.ReLU(),
        )

        self.decoderConv3 = nn.Sequential(
            BasicConv2d(in_channels=128, out_channels=64,
        ...
        self.decoderConv1 = nn.Sequential(
            ...
            nn.Upsample(scale_factor=2),
            nn.Conv2d(in_channels=16, out_channels=out_channels,
                      kernel_size=3, stride=1, padding=1),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        x = x.view(x.shape[0], 128, self.width, self.width)
        # print(x.shape)
...
class ConvContrastive(nn.Module):
    def __init__(self, in_channels, resolution):
        ...
        self.Latent1 = nn.Linear(10, 10)
        self.Projector = Projector(10*2)
...
class VAEConvContrastive(nn.Module):
    def __init__(self, in_channels, resolution):
        ...
        self.Encoder = ConvEncoder(
            in_channels=in_channels, resolution=resolution)
        self.mu1 = nn.Linear(10, 10)
        self.var1 = nn.Linear(10, 10)

        self.mu2 = nn.Linear(10, 10)
        self.var2 = nn.Linear(10, 10)
        self.Projector = Projector(10*2)
        self.Latent2 = nn.Linear(10, 10)
        self.Decoder = ConvDecoder(
            out_channels=in_channels, resolution=resolution)
...
def testContrastive():
    input1 = torch.rand((10,3,84,84))
    print(input1.shape)
    model = VAEConvContrastive(3,84)
